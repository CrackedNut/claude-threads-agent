...
### Added
- **`allChannels` mode (Mattermost).** New per-platform `allChannels: true` makes the bot answer `@mentions` from `allowedUsers` in *every* channel it is a member of — including group/DM channels — not just the configured `channelId`. Every channel then behaves like the home channel: a root `@mention` starts that channel's shared channel-mode session (keyed `platformId:channelId`) and replies land at the channel root; `!thread` opts a conversation out into its own thread, anywhere. The reply convention changed to make this routable — channel-mode call sites pass the session's channelId as the reply target and the platform clients resolve channel targets to root-less posts in the right channel (Mattermost via home-id match / an intake-learned threadRoot→channel index / a `GET /channels` fallback; Slack via home-id match). This also eliminates the "Invalid RootId" 400 class by construction. Typing indicators, file uploads, the `!thread` anchor + `-history` context, and the MCP permission child all route per-channel. The sticky message and missed-message recovery remain exclusive to the home `channelId`. Authorization is unchanged: only `allowedUsers` can start sessions anywhere. Default off.

### Fixed
- **`!cd` no longer kills the session it just respawned.** `restartClaudeSession` now detaches the replaced Claude CLI's event/exit/rate-limit listeners before killing it, so the dying process's asynchronous exit event can't reach `handleExit` and tear down the freshly-rebuilt session after a post-helper's defensive state transition drops the `restarting` guard.
- **Channel-mode sessions survive bot restarts.** `resumeSession` skipped the thread-exists check for channel-mode sessions, whose `threadId` is the channelId — `GET /posts/<channelId>` is always a 404, which previously dropped every channel session on restart with "Thread deleted, skipping resume".
- **Channel "black hole" after a session is soft-deleted.** `resumePausedSession` now uses the same any-state lookup as the message-handler's paused check; the old `load()`-filtered lookup excluded soft-deleted sessions while the routing check included them, so every channel post was classified "paused", resumed into nothing, and never fell through to start a fresh session — wedging the channel until the 3-day purge. The paused path also now posts visible feedback instead of silently consuming commands and bare mentions.
- **Session working directory honors `config.workingDir`.** Sessions seeded their cwd from `process.cwd()` — the daemon's launch dir (the source checkout) — and ignored the configured `workingDir`, so every session started in the repo directory instead of the user's intended directory. Resume is unaffected (persisted sessions keep their stored cwd).
...
- **Auto-detected channel-mode + shared collaborative sessions.** The bot now picks thread vs. channel reply model per inbound message — no config flag needed. If you @mention the bot as a *reply inside an existing thread*, the bot creates a thread-mode session keyed by `platformId:threadId` and replies in the thread (historical behavior, unchanged). If you @mention at *channel root* (no parent thread), the bot creates a shared channel-mode session keyed by `platformId:channelId` and replies as channel root posts. The two modes coexist freely in the same channel: one shared channel session at the root plus any number of threaded sessions inside threads. Channel-mode sessions are SHARED across every allowed user in the channel — Alice and Bob mentioning the bot in the same channel land in the same Claude conversation. To let Claude disambiguate speakers, inbound messages are now prefixed with `@username: ` in channel mode (always) and in thread mode (only when the speaker is not the session owner, so solo thread sessions stay clean). New `Session.mode`/`channelId` fields plus `PersistedSession` round-trip; new `SessionRegistry.findByChannelId` lookup; new `directReplyTo` in `message-handler.ts` for the handful of direct `createPost` calls (kill confirmations, error notifications, "Mention me with your request") so they land at the channel root rather than as stray thread replies in a channel-mode flow. Default behavior for existing deployments is unchanged.
